...
window.Locale.define('en-US', 'DatePicker', {
...
var Picker = new Class({
	...
	eDPosition: function(x, y){
		...
		// autodetect if it has fixed parent and set
		// position fixed which prevents it from scrolling
		var hasFixedParent = element.getParents().some(function (item){
			if (item.getStyle('position') == 'fixed') return true;
		});

		if(hasFixedParent){
			$$('.'+this.options.pickerClass).setStyle('position', 'fixed');
		}
		...
		var getAbsoluteCoords = function(obj) {
			...
			r.left = objPos[0];
			r.top = objPos[1];
			r.right = r.left + obj.getSize().x;
			r.bottom = r.top + obj.getSize().y;

			return r;
		}

		var elementCoords = getAbsoluteCoords(element);
